...
module.exports = async (req, res) => {
  ...
  try {
    const body = req.body || {};
    const { adm, dem, sal, motivo, aviso, ferias, nome, email } = body;
    ...
    if (!email || !/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email)) {
      res.status(400).json({ erro: 'Informe um e-mail válido para gerar o pagamento.' });
      return;
    }
    ...
    const mpRes = await fetch('https://api.mercadopago.com/v1/payments', {
      ...
      body: JSON.stringify({
        ...
        payer: {
          ...
          last_name: (nome || '').split(' ').slice(1).join(' ') || 'Rescisao'
        },
        metadata: { adm, dem, sal: String(sal), motivo, aviso, ferias, token, nome, email }
      })
    });
    ...
    if (!mpRes.ok) {
      ...
      res.status(500).json({ erro: 'Falha ao criar a cobrança no Mercado Pago.', detalhe: (data.message || JSON.stringify(data)) + causa + ' | token em uso começa com: ' + tokenTipo });
      return;
    }
