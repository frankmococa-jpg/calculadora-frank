...
function calcular(admStr, demStr, sal, motivo, aviso, feriasVenc, recebido) {
  ...
  // Comparação: quanto a empresa pagou x quanto era devido
  let comparativo = null;
  const pago = parseFloat(recebido);
  if (!isNaN(pago) && pago >= 0) {
    const diff = total - pago;
    comparativo = { pago, diferenca: diff, faltou: diff > 1 };
  }

  return { verbas, total, nota, totalDias, anosCompletos, meses, comparativo };
}
